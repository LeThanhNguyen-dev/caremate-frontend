...
const SingleServiceProgressTracker = ({ booking, onProgressChanged }: Props) => {
  ...
  const updateStatus = async (nextStatus: string, successMessage: string) => {
    try {
      ...
      await caremateApi.updateBookingStatus(booking.id, { status: nextStatus });
    ...
    <div className="mt-12 overflow-hidden rounded-xl border border-slate-50 bg-white shadow-[0_32px_80px_rgba(0,0,0,0.06)]">
      <div className="bg-slate-900 p-8 text-white">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="rounded-full bg-white/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em]">
            {terminalLabels[booking.status]?.label || steps[currentIndex]?.label || booking.status}
          </div>
        </div>
