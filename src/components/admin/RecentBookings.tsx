...
const RecentBookings = () => {
    ...
    return (
        <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-50 flex-1 overflow-hidden">
            <div className="p-6 flex justify-between items-center border-b border-gray-50">
                <h3 className="text-lg font-bold text-gray-900">Recent Service Bookings</h3>
                <button className="text-sm font-medium text-[#4F8CFF] hover:text-blue-600 transition-colors">
                    View All
                </button>
            </div>
                        ...
                        {bookings.map((booking) => (
                            <tr key={booking.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-6 py-4 font-medium text-gray-900 flex items-center gap-3">
                                    <img src={booking.avatar} alt="" className="w-8 h-8 rounded-full object-cover" />
                                    {booking.parent}
                                </td>
                                <td className="px-6 py-4">{booking.nurse}</td>
