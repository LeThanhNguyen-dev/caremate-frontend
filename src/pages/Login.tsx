...
const Login = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast(t('toast.welcomeBack', { name: user.username }), 'success');
            ...
            showToast(getErrorMessage(err, t('toast.invalidLogin')), 'error');
        } finally {
            setLoading(false);
        }
    };
            ...
            >
                <div className="hidden lg:flex flex-col justify-between p-16 bg-[#111827] text-white relative">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="mt-24">
                            <h2 className="text-5xl font-black leading-tight text-white" dangerouslySetInnerHTML={{ __html: t('login.welcomeBackTitle') }}></h2>
                            <p className="mt-6 text-white/40 font-medium leading-relaxed max-w-sm">{t('login.welcomeBackDesc')}</p>
                        </div>
                    </div>
                    <div className="relative z-10 flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-white/20">
                        <span className="h-px w-8 bg-white/10"></span> {t('login.luxuryCare')}
                    </div>
                </div>
                ...
                <div className="p-6 sm:p-10 lg:p-20 bg-white">
                    <div className="max-w-sm mx-auto">
                        ...
                        <h1 className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">{t('login.title')}</h1>
                        <p className="mt-4 text-sm font-bold text-slate-400">{t('login.noAccount')} <Link to="/register" className="text-brand font-black hover:underline">{t('login.registerNow')}</Link></p>

                        <form onSubmit={handleSubmit} className="mt-8 sm:mt-12 space-y-8 sm:space-y-10">
                            <div className="space-y-6">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t('login.emailLabel')}</label>
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t('login.passwordLabel')}</label>
                                        <Link to="/forgot-password" className="text-[10px] font-black uppercase tracking-widest text-slate-300 hover:text-brand transition-colors">{t('login.forgotPassword')}</Link>
                                    </div>
                                    <div className="relative group">
